...
export default function WatchlistScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      if (token) {
        const data = await getWatchlist(activeTab || undefined);
        setMovies(data);
      } else {
        setMovies([]);
      }
  ...
  const handleStatusChange = async (movieId, newStatus) => {
    try {
      await updateWatchStatus(movieId, newStatus);
      setMovies((prev) =>
        prev.map((m) =>
          m._id === movieId ? { ...m, watchStatus: newStatus } : m
        )
      );
    } catch {}
  };
  ...
  const statusColor = (s) => {
    switch (s) {
      case 'watching': return '#2980B9';
      case 'plan_to_watch': return '#D4AC0D';
    ...
    >
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardMeta}>
          ...
          <View style={[styles.statusTag, { backgroundColor: statusColor(item.watchStatus) + '20' }]}>
            <Text style={[styles.statusTagText, { color: statusColor(item.watchStatus) }]}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity style={styles.actionBtn} onPress={() => handleStatusChange(item._id, 'watching')}>
            <Text style={[styles.actionBtnText, item.watchStatus === 'watching' && styles.actionBtnActive]}>Watching</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={() => handleStatusChange(item._id, 'plan_to_watch')}>
            <Text style={[styles.actionBtnText, item.watchStatus === 'plan_to_watch' && styles.actionBtnActive]}>Plan to Watch</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={() => handleStatusChange(item._id, 'completed')}>
            <Text style={[styles.actionBtnText, item.watchStatus === 'completed' && styles.actionBtnActive]}>Completed</Text>
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.centeredContainer}>
          <MaterialIcons name="bookmark-border" size={64} color="#BCAAA4" />
          <Text style={styles.emptyText}>Your watchlist is empty</Text>
          <Text style={styles.emptySubText}>Add movies to keep track of what you want to watch</Text>
        ...
        <View style={styles.drawerOverlay}>
          ...
          <View style={styles.drawerPanel}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.drawerScroll}>
              ...
              <TouchableOpacity style={styles.menuItemRow} onPress={() => { setMenuVisible(false); router.push('/(tabs)/favorites'); }}>
                <Text style={styles.menuItemText}>FAVORITES</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.menuItemRow, { borderBottomColor: '#D35400' }]} onPress={() => setMenuVisible(false)}>
                <Text style={[styles.menuItemText, { color: '#D35400' }]}>WATCHLIST</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  tabsRow: { backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#F5EBE6', height: 44 },
  tabsScroll: { paddingHorizontal: 20, alignItems: 'center', gap: 16 },
  tab: { height: '100%', justifyContent: 'center', borderBottomWidth: 3, borderBottomColor: 'transparent', paddingHorizontal: 4 },
  tabActive: { borderBottomColor: '#D35400' },
  tabText: { fontSize: 14, fontWeight: '600', color: '#8D6E63' },
  tabTextActive: { color: '#D35400', fontWeight: '700' },
  ...
  actionsRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8, flexWrap: 'wrap' },
  actionBtn: { backgroundColor: '#F5EBE6', borderRadius: 4, paddingHorizontal: 8, paddingVertical: 3 },
  actionBtnText: { fontSize: 10, fontWeight: '600', color: '#8D6E63' },
  actionBtnActive: { color: '#D35400' },
  // Drawer
